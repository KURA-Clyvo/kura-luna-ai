...
3. **Lembretes de vacinas via WhatsApp** — lê vacinas próximas do vencimento no Oracle, cria registros de notificação e dispara mensagens pelo Twilio Sandbox. O `ORA-00904` que a view antiga produzia foi corrigido (`LU-02`/`LU-03`, V21). ⚠️ O scheduler diário é **opcional e desligado por padrão** (`LUNA_SCHEDULER_ENABLED=false`); o gatilho manual (`POST /jobs/lembrete-vacina/executar`) sempre existe. **O ciclo completo de envio não foi verificado ao vivo** nesta máquina por falta de credencial Twilio no processo — ver `docs/IA_DEFINICAO.md` §9 e o ledger `lu-16-revisao.md`.
4. **Identificação de raça por foto** — 🟡 **extra opcional, NÃO faz parte da imagem padrão.** Detecta cão/gato com YOLOv8n e classifica a raça com MobileNetV3 (35 raças), só quando a imagem é construída com `--build-arg WITH_VISION=true` (`Dockerfile`). O build padrão (`WITH_VISION=false`, o que o `docker-compose.yml` do `DevOps-Cloud` usa) **não inclui** este pipeline e fica < 800 MB; com visão, requer também um checkpoint de classificador que **não está versionado** — ver `docs/IA_DEFINICAO.md` §9.2.
